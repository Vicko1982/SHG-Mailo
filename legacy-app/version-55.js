...
(() => {
  ...
  const chatRows55 = () => state.tasks.filter(task => !isTaskDeleted(task) && canCurrentUserAccessTask(task) && participant55(task) && (showDoneChats || unreadOnlyChats || task.status !== 'done'));
  const formatDuration55 = seconds => `${Math.floor(seconds / 60)}:${String(Math.max(0, seconds % 60)).padStart(2, '0')}`;
  const latestChatTime55 = task => Math.max(new Date(task.updated || 0).getTime() || 0, ...(task.comments || []).map(comment => new Date(comment.createdAt || 0).getTime() || 0));
  const latestChatComment55 = task => [...(task.comments || [])].sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0))[0];

  function matchingChatRows55() {
    const query = String(window.MAILO_CHAT_QUERY || '').trim().toLowerCase();
    return chatRows55().filter(task => !unreadOnlyChats || unread55(task) > 0).filter(task => !query || `${task.id} ${task.title}`.toLowerCase().includes(query)).sort((a, b) => latestChatTime55(b) - latestChatTime55(a));
  }
  ...
  window.renderTaskChat = () => {
    if (!chatView) return;
    const tasks = matchingChatRows55(), active = state.tasks.find(task => task.id === window.MAILO_ACTIVE_CHAT && participant55(task) && (showDoneChats || unreadOnlyChats || task.status !== 'done')) || null;
