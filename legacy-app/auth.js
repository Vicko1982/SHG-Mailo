  ...
  if ('caches' in window) {
    caches.keys()
      .then(keys => Promise.all(keys
        .filter(key => key.startsWith('shg-task-manager-') && key !== 'shg-task-manager-v277')
        .map(key => caches.delete(key))))
      .catch(() => {});
  }
  if ('serviceWorker' in navigator && location.protocol.startsWith('http')) {
    addEventListener('load', () => navigator.serviceWorker.register('./service-worker.js?v=277').catch(() => {}), { once: true });
  }
